...
export default function MapView({ selectedDay, days, onPoiClick, isMobile = false, bottomOffset = 0 }) {
  ...
  const navLabel = overviewMode ? '全程导航' : `${selectedDay?.title?.split(' · ')[0] || 'Day'} 导航`
  const isTouch = /Mobi|Android|iPhone/i.test(navigator.userAgent)
  const btnPad = isMobile ? '12px 18px' : '10px 18px'
  const bottomBase = bottomOffset + 20
  ...
  return (
    <div style={{ flex: 1, position: 'relative', background: '#000000', height: '100%' }}>
      ...
      <div style={{
        position: 'absolute',
        top: isMobile ? 'env(safe-area-inset-top, 12px)' : 16,
        left: '50%',
        ...
        <div style={{
          ...
          {[['全程', true], ['今日', false]].map(([label, mode]) => (
            <button
              key={label}
              ...
              style={{
                ...
                background: overviewMode === mode ? '#181818' : '#FFFFFF',
                color: overviewMode === mode ? '#FFFFFF' : '#8F8F8F',
                ...
                borderLeft: mode === false ? '1px solid #D2D2D2' : 'none',
        ...
        {!overviewMode && selectedDay && (
          <div style={{
            ...
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: selectedDay.color, display: 'inline-block', flexShrink: 0 }} />
            <span style={{ fontSize: isMobile ? 12 : 13, fontWeight: 600, color: '#181818', letterSpacing: '0.13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {isMobile ? selectedDay.title.split(' · ')[0] : selectedDay.title}
            </span>
            {selectedDay.distance > 0 && !isMobile && (
              <>
                <span style={{ width: 1, height: 14, background: '#D2D2D2', display: 'inline-block' }} />
                <span style={{ fontFamily: 'var(--body-font)', fontSize: 10, letterSpacing: '1px', textTransform: 'uppercase', color: '#8F8F8F' }}>
        ...
        {overviewMode && (
          <div style={{
            ...
            boxShadow: 'rgb(0,0,0) 0px 0px 0px 1px',
            pointerEvents: 'none',
            fontSize: isMobile ? 12 : 13, fontWeight: 600, color: '#181818',
            whiteSpace: 'nowrap',
          }}>
            {isMobile ? '北疆 14 天' : '北疆 14 天完整路线'}
          </div>
        )}
      </div>

      {/* 右下：高德导航按钮 */}
      <div style={{ position: 'absolute', bottom: bottomBase, right: 16, zIndex: 10 }}>
        <button
          ...
          style={{
            ...
            fontFamily: 'var(--ferrari-sans)',
            fontSize: isMobile ? 13 : 12, fontWeight: 500, letterSpacing: '0.5px',
          ...
          onMouseEnter={e => { if (!copied && !isMobile) e.currentTarget.style.background = '#B01E0A' }}
          onMouseLeave={e => { if (!copied && !isMobile) e.currentTarget.style.background = '#DA291C' }}
        >
          {copied ? (
            <>✓ {isTouch ? '已打开' : '链接已复制'}</>
          ) : (
        ...
        {!isMobile && (
          <div style={{
            fontFamily: 'var(--body-font)', fontSize: 9, letterSpacing: '0.5px',
            color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginTop: 5, textTransform: 'uppercase',
          }}>
            {isTouch ? '点击打开高德' : '点击复制 · 粘贴至高德'}
          </div>
        )}
      </div>
      ...
      {!isMobile && (
        <div style={{
          ...
          background: '#FFFFFF', borderRadius: 2, padding: '12px 16px',
          boxShadow: 'rgb(0,0,0) 0px 0px 0px 1px',
          fontFamily: 'var(--ferrari-sans)', pointerEvents: 'none',
        }}>
          <div style={{ fontFamily: 'var(--body-font)', fontSize: 9, letterSpacing: '1px', textTransform: 'uppercase', color: '#8F8F8F', marginBottom: 8 }}>
            Legend
          </div>
          {overviewMode
            ? days.filter((_, i) => i % 2 === 0).slice(0, 7).map(day => (
              <div key={day.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: '#181818', marginBottom: 3, fontWeight: 500 }}>
                <span style={{ width: 20, height: 2.5, background: day.color, display: 'inline-block', borderRadius: 1, flexShrink: 0 }} />
                <span style={{ color: '#666' }}>{day.title.split(' · ')[0]}</span>
                <span style={{ color: '#181818' }}>{day.title.split(' · ')[1] || ''}</span>
              </div>
            ))
            : [['🏔️', '景点'], ['🏨', '住宿'], ['⛽', '加油站'], ['🍜', '餐厅']].map(([ic, lb]) => (
              <div key={lb} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#181818', marginBottom: 4, fontWeight: 500 }}>
